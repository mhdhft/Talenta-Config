...
import { uploadNewDocument, ApiError } from "@/lib/api"

export function StepUploadNew() {
  const { analysisRunId, uploadNewDocument: setNewDocUploaded } = useStage()
  ...
  async function handleSubmit() {
    ...
    setError(null)
    try {
      const res = await uploadNewDocument(analysisRunId, file)
      setNewDocUploaded(res.filename)
    } catch (err) {
  ...
  return (
    <Card>
      <CardHeader>
        ...
        <CardDescription>
          Upload dokumen data karyawan terbaru (.xlsx atau .csv) untuk memulai proses
          pemetaan.
        </CardDescription>
      </CardHeader>
      <CardContent className="space-y-4">
        <div>
          <Label htmlFor="new-doc-upload" className="mb-2 block">
